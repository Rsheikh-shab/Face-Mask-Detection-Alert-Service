...
from loader import load_pytorch_model, pytorch_inference
    ...
    for idx in keep_idxs:
        ...
        if draw_result:
            if class_id == 1:
                ...
                toast = ToastNotifier()
                toast.show_toast("No Mask","Person was detected without mask",duration=1)
                while toast.notification_active(): time.sleep(1)
    ...
    while status:
        ...
        if (status):
            inference(img_raw,
                      ...
                      draw_result=True,
                      show_result=False)
            cv2.imshow('Face Mask Alert', img_raw[:, :, ::-1])
...
if __name__ == "__main__":
    ...
    if args.img_mode:
        ...
        if args.video_path == '0':
            video_path = 0
        run_on_video(video_path, 'output.avi', conf_thresh=0.5)
